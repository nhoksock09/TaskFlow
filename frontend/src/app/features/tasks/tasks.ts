...
export type TaskUrgencyFilter = TaskFilterStatus;
...
export class Tasks implements OnInit {
  ...
  priorityOptions: SelectOption<TaskPriority>[] = [
    ...
    { label: 'TASKS.FILTERS.LOW', value: TaskPriority.LOW }
  ];
  statusOptions: SelectOption<TaskStatus>[] = [
    { label: 'TASKS.STATUS.TODO', value: TaskStatus.TODO },
    { label: 'TASKS.STATUS.IN_PROGRESS', value: TaskStatus.IN_PROGRESS },
    { label: 'TASKS.STATUS.COMPLETED', value: TaskStatus.COMPLETED }
  ];

  // ── Formly task form ───────────────────────────────────────────────────────
  taskForm = new FormGroup({});
  taskModel: TaskFormModel = { title: '', description: '', priority: TaskPriority.MEDIUM, dueDate: '', status: TaskStatus.TODO };
      ...
      validators: {
        pastDate: {
          expression: (c: AbstractControl) => {
            ...
            // Allow unchanged past dates when editing an existing task
            if (this.editingTaskId && this.originalTaskSnapshot?.dueDate) {
              const originalMs = new Date(this.originalTaskSnapshot.dueDate).getTime();
              const currentMs = new Date(c.value).getTime();
              if (Math.abs(currentMs - originalMs) < 60000) return true;
            }
  ...
  activeStatusFilter: TaskUrgencyFilter = TaskFilterStatus.ALL;
  ...
  originalTaskSnapshot: TaskFormModel | null = null;
  ...
  getTasksCountByStatus(status: TaskStatus): number {
  ...
  openAddModal(defaultStatus?: TaskStatus) {
    this.resetForm();
    this.taskModel = { title: '', description: '', priority: TaskPriority.MEDIUM, dueDate: '', status: defaultStatus || TaskStatus.TODO };
  ...
  getFormattedDateForInput(dateVal: string | Date): string {
    if (!dateVal) return '';
  ...
  translateFilters() {
    this.translateService.get([
      ...
      this.priorityFilterOptions = [
        ...
        { label: translations['TASKS.FILTERS.HIGH'], value: TaskPriority.HIGH },
        { label: translations['TASKS.FILTERS.MEDIUM'], value: TaskPriority.MEDIUM },
        { label: translations['TASKS.FILTERS.LOW'], value: TaskPriority.LOW }
      ];
      this.statusFilterOptions = [
        { label: translations['TASKS.FILTERS.ALL_STATUS'], value: TaskFilterStatus.ALL },
        { label: translations['TASKS.FILTERS.OVERDUE'], value: TaskFilterStatus.OVERDUE },
        { label: translations['TASKS.FILTERS.DUE_SOON'], value: TaskFilterStatus.DUE_SOON }
      ];
  ...
  /** Triggered only on Enter key press — commits search */
  onSearch() {
  ...
  get allFilteredTasks(): Task[] {
    ...
    if (this.activeStatusFilter === TaskFilterStatus.OVERDUE) {
      filtered = filtered.filter(task => this.isOverdue(task));
    } else if (this.activeStatusFilter === TaskFilterStatus.DUE_SOON) {
  ...
  hasTaskChanged(): boolean {
    if (!this.originalTaskSnapshot) return true;
    const original = this.originalTaskSnapshot;
    const current = this.taskModel;
    const originalDate = original.dueDate ? new Date(original.dueDate).toISOString() : '';
    const currentDate = current.dueDate ? new Date(current.dueDate).toISOString() : '';
    return (
    ...
    if (this.taskForm.valid) {
      ...
      if (this.editingTaskId) {
        if (!this.hasTaskChanged()) {
          this.toastService.info('TASKS.TOAST.NO_CHANGES');
          return;
        }
        this.taskService.updateTask(this.editingTaskId, taskData)
          ...
          .subscribe({
            ...
            error: () => this.toastService.error('TASKS.TOAST.UPDATE_FAILED')
          });
      } else {
        this.taskService.createTask(taskData)
          ...
          .subscribe({
            ...
            error: () => this.toastService.error('TASKS.TOAST.CREATE_FAILED')
          });
      }
    } else {
  ...
  editTask(task: Task) {
    ...
    this.originalTaskSnapshot = { ...this.taskModel };
    ...
    if (this.taskToDelete && this.taskToDelete._id) {
      this.taskService.deleteTask(this.taskToDelete._id)
        ...
        .subscribe({
          ...
          error: () => this.toastService.error('TASKS.TOAST.DELETE_FAILED')
        });
    }
  }
